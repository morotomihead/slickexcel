    ...
    */
    ...
    function HeaderFilter(options) {
        ...
        function showFilter(e) {
            ...
            var filterOptions = "<label><input type='checkbox' value='-1' />(Select All)"+ " (Total Unique : " + filterItems.length +")" + "</label>";
            ...
            for (var i = 0; i < filterItems.length; i++) {
                ...
                filterOptions += "<label><input type='checkbox' value='" + i + "'"
                                 ...
                                 + "/>" + filterItems[i] + ":( "  + filterItemsUnique[filterItems[i]] + " )</label>";
            }
        ...
        function getFilterValues(dataView, column) {
            ...
            //check Array's Value Type (number Type,and like number Type)
            if (isNaN(seen[0]) === false) {
                return _.sortBy(seen, function (v) { return parseInt(v); }); //case number
            } else {
                return _.sortBy(seen, function (v) { return v; }); //case String
            }
        }
        ...
        function getFilterUniqueCount(dataView, column) {
            var counts = {};

            for (var i = 0; i < dataView.getLength() ; i++) {
                counts[dataView.getItem(i)[column.field]] = 1 + (counts[dataView.getItem(i)[column.field]] || 0);
            }
            return counts;
        }

        function getAllFilterUniqueCount(data, column) {
            var counts = {};

            for (var i = 0; i < data.length; i++) {
                counts[data[i][column.field]] = 1 + (counts[data[i][column.field]] || 0);
            }
            return counts;
        }
